 ...
 * <p>
 * The layout used is BorderLayout and the CENTER component is dedicated to the user (the other 4 places
 * (NORTH, EAST, WEST and SOUTH are used to display the toolbar panels)).
 *
 * <p align="center">
 ...
 * VLToolbars are registered (given a name and made know to their container by
 * {@link #registerToolBar(VLToolBar)} ).
 *
...
public class ToolBarContainer extends JPanel {
    ...
    private Map componentsByContraints = new HashMap(); // this is private in borderlayout, so we need to keep
                                                        // a copy here
    ...
    /**
     * Creates a default ToolBarContainer with preinstalled toolbar panels on the borders with the LEADING
     * alignment.
     * <p>
     * The toolbarPanels are then accessible with getToolBarPanelAt(constraints) where constraints values are
     * BorderLayout.NORTH, EAST, WEST and SOUTH.
     */
    ...
    /**
     * Creates a default ToolBarContainer with preinstalled toolbar panels on the borders with the specified
     * alignment The value of the alignment argument must be one of <code>FlowLayout.LEFT</code>,
     * <code>FlowLayout.RIGHT</code>, <code>FlowLayout.CENTER</code>, <code>FlowLayout.LEADING</code>, or
