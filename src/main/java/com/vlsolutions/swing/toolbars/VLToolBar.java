 ...
 * <p>
 * Toolbars can now be collapsed (to gain space on screen). this property is disabled by default to keep
 * compatibility with previous releases. For a better usage of collapsing it is better to set a tooltip text
 * on the toolbar (it will be propagated to the gripper, which will be the only one component visible when
 * collapsed).
 *
...
public class VLToolBar extends JPanel {
    ...
    /**
     * Constructs a toolbar with a null name (this toolbar will not be able to write its structure in a XML
     * encoder, unless you call setName(String) with a non null name before saving). ).
     */
    public VLToolBar() {
        this(null);
    }
     ...
     * <p>
     * This method can be overriden to replace the provided "look and feel" which uses the follwing
     * configuration :
     * <ul>
    ...
    /**
     * This method is invoked upon adding a button to the toolbar. It can be overriden to provide another look
     * or feel.
     * <p>
     ...
     * <p>
     * For a better user experience, please note to install an empty border of the same Insets than this one
     * (otherwise, there will be a size change of the toolbar at the beginning of the drag and after the
     * drop).
     */
