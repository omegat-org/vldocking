...
public class DockViewTitleBarUI extends PanelUI implements PropertyChangeListener {
    ...
    private static boolean useCustomPaint =
        !System.getProperty("os.name").contains("OS X") && !UIManager.getBoolean("DockViewTitleBar.disableCustomPaint");
    ...
    /*
     * This ancestor listener is required as buttons may change according to container hierarchy. The first example is
     * when a dockable is added to a floating + compund dockable, the attach button (which usually becomes visible as
     * the dockable is in the floating state) has to be hidden (tech choice : we don't want to allow attaching a single
     * child of a compound dockable)
     *
     */
    ...
    public void propertyChange(PropertyChangeEvent e) {
        ...
        if (pName.equals("dockable")) {
            ...
            if (notification) {
                ...
                titleBar.putClientProperty("titlebar.notification", true);
                titleBar.setOpaque(true);
            }
            else {
                ...
                titleBar.putClientProperty("titlebar.notification", false);
    ...
    /**
     * Update the buttons to track state changes (for example, the maximize button can become "restore" when the view is
     * maximized.
     */
    ...
    public void paint(Graphics g, JComponent c) {
        ...
        if (useCustomPaint) {
            ...
            if (titleBar.getClientProperty("titlebar.notification") == Boolean.TRUE) {
                background = UIManager.getColor("DockingDesktop.notificationColor");
            }
            ...
            if (tb.isActive()) {
                if (UIManager.getLookAndFeel().getName().contains("Substance")) {
                    g2
                        .setPaint(new GradientPaint(i.left + w, 0, UIManager.getColor("controlLtHighlight"),
                            tb.getWidth(), 0, UIManager.getColor("controlHighlight"))); // Active substance
                }
                else {
                    g2
                        .setPaint(new GradientPaint(i.left + w, 0, UIManager.getColor("controlShadow"), tb.getWidth(),
                            0, UIManager.getColor("controlHighlight"))); // Active other
                }
            }
            else {
                g2
                    .setPaint(new GradientPaint(i.left + w, 0, /* UIManager.getColor("Panel.background") */ background,
