 ...
 * dockable, based on a relative positionning.
 *
 * @author Lilian Chamontin, vlsolutions.
 * @version 1.0
 * @update 2005/10/10 Lilian Chamontin : improved heuristics of resizing (best matching of the appropriate
 *         SplitContainer)
...
class RelativeDockingUtilities {
    ...
    /*
     * This class is an extraction of DockingDesktop - we will enhance (and complexify) the algorithm later,
     ...
     */
    private static SplitContainer findBestContainer(Container ancestorContainer,
        ...
        if (splitComp instanceof SplitContainer) { // 2006/09/12
            /*
             * Try to find a larger splitcontainer that still fits well with the position. for example :
             *
             * [1|___2_____] [1|[3|[4|5]]]
             *
             * if we remove "2", what remains is [1|[3|[4|5]]] and on restoring, the selected split is [4|5],
             * although [3|[4|5]] would be a much better choice.
             *
            ...
            while (found && splitComp.getParent() instanceof SplitContainer) {
                ...
                boolean sameOrientation = ((SplitContainer) splitComp).getOrientation() == parentSplit
                        .getOrientation();
                if (sameOrientation) {
        ...
        /*
         * while (splitComp != null && splitComp != ancestorContainer && ! (splitComp instanceof
         * SplitContainer)) { splitComp = splitComp.getParent(); }
         */
        ...
        if (splitComp instanceof SplitContainer) {
            ...
            /*
             * boolean anchoredTop = y < 5; //2005/11/08 enhanced ! boolean anchoredLeft = x < 5; boolean
             * anchoredBottom = Math.abs(y + h - ancestorContainer.getHeight()) < 5; boolean anchoredRight =
            ...
            return findBetterContainer((SplitContainer) splitComp, ancestorContainer, anchoredTop,
    ...
    private static SplitContainer findBetterContainer(SplitContainer split, Container dockingPanel,
            boolean anchoredTop, boolean anchoredLeft, boolean anchoredBottom, boolean anchoredRight) {
        ...
        /*
         * System.out.println("global anchors " + anchoredTop + " " + anchoredLeft + " " + anchoredBottom +
         * " " + anchoredRight);
         */
    ...
    /*
     * private static boolean [] findAnchors(Component comp, Container container){ Rectangle r =
     * SwingUtilities.convertRectangle(comp, new Rectangle(0, 0, comp.getWidth(), comp.getHeight()),
     * container); int x = container.getX(); int y = container.getY(); int w = container.getWidth(); int h =
     * container.getHeight(); boolean [] anchors = new boolean [4]; anchors[0] = Math.abs( y - r.y) < 5;
     * anchors[1] = Math.abs(x - r.x) < 5; anchors[2] = Math.abs(y + h - r.y - r.height) < 30; anchors[3] =
     ...
     *
     * @param base
     *            the component to find anchors for
     * @param ancestorContainer
     *            the top level ancestor used to stop searching
     */
    ...
    public static SingleDockableContainer insertDockable(Container relativeAncestorContainer,
        ...
        if (relativeAncestorContainer.getComponentCount() == 0) { // empty docking panel
            dockableContainer = DockableContainerFactory.getFactory().createDockableContainer(dockable,
            ...
            if (split != null) {
                // ok we've found one
                /*
                 * Rectangle splitRect = SwingUtilities.convertRectangle(split, new Rectangle(0, 0,
                 * split.getWidth(), split.getHeight()), relativeAncestorContainer);
                 */
                ...
                // is nearer of center
                /*
                 * boolean anchoredTop = Math.abs( y - splitRect.y) < 5; boolean anchoredLeft = Math.abs(x -
                 * splitRect.x) < 5; boolean anchoredBottom = Math.abs(y + h - splitRect.y - splitRect.height)
                 * < 30; boolean anchoredRight = Math.abs(x + w - splitRect.x - splitRect.width) < 5;
                 */
                ...
                Point leftCenter = SwingUtilities.convertPoint(left, left.getWidth() / 2,
                        left.getHeight() / 2, relativeAncestorContainer);
                Point rightCenter = SwingUtilities.convertPoint(right, right.getWidth() / 2,
                ...
                // 2006/09/12
                /*
                 * If the split matches the size of the component, then we'll have to split it (and not split
                 * one of its children)
                 */
                    ...
                    if (anchoredTop) {
                        ...
                        if (anchoredRight) { // 2005/10/10
                            ...
                            if (bestComp == top) {
                                ...
                                SwingUtilities
                                        .invokeLater(new SplitResizer(newSplit, h / (float) splitBottom));
                            }
                    ...
                    if (anchoredTop) {
                        if (anchoredLeft) {
                            ...
                            if (bestComp == left) {
                                ...
                                newSplit.setRightComponent((Component) dockableContainer);
                                SwingUtilities.invokeLater(new SplitResizer(split,
                                        (split.getWidth() - xRight) / (float) split.getWidth()));
                                SwingUtilities.invokeLater(
                                ...
                                SwingUtilities
                                        .invokeLater(new SplitResizer(newSplit, w / (float) splitRight));
                            }
                        } else {
                            // just anchored top
                            if (splitWidthRatio < 0.1f && split.getHeight() > h) { // 2006/09/12
                        ...
                        if (anchoredRight) {
                            ...
                            DockingUtilities.replaceChild(split.getParent(), split, newSplit);
                            newSplit.setTopComponent((Component) dockableContainer);
                            newSplit.setBottomComponent(split); // todo : check this case : why is the new
                                                                // component on top
                            SwingUtilities.invokeLater(new SplitResizer(newSplit, proportionH));
                        } else {
                            // just left
                        ...
                        if (splitWidthRatio < 0.1f && split.getHeight() > h) { // 2006/09/12
                            ...
                            newSplit.setBottomComponent(split);
                            SwingUtilities
                                    .invokeLater(new SplitResizer(newSplit, h / (float) split.getHeight()));
                        } else {
                            SplitContainer newSplit = new SplitContainer(JSplitPane.HORIZONTAL_SPLIT);
                            DockingUtilities.replaceChild(split, right, newSplit);
                            newSplit.setLeftComponent(right);
                ...
                if (anchoredTop) {
                    if (anchoredLeft) {
                        if (anchoredBottom) {
                            hSplitAndResize(singleComp, (Component) dockableContainer, singleComp,
                                    proportionW);
                        } else if (anchoredRight) { // top + left + right == TOP
                            vSplitAndResize(singleComp, (Component) dockableContainer, singleComp,
                                    proportionH);
                        ...
                        if (anchoredBottom) { // top + right + bottom == on the right
                            hSplitAndResize(singleComp, singleComp, (Component) dockableContainer,
                                    1 - proportionW);
                        } else { // top + right
                            if (w > h) { // on top
                                vSplitAndResize(singleComp, (Component) dockableContainer, singleComp,
                    ...
                    if (anchoredBottom) { // left + bot
                        if (anchoredRight) { // == bottom
                            vSplitAndResize(singleComp, singleComp, (Component) dockableContainer,
                                    1 - proportionH);
                        } else { // left + bottom
                            if (w > h) { // bottom
                                vSplitAndResize(singleComp, singleComp, (Component) dockableContainer,
    ...
    public static DockingActionEvent getInsertionDockingAction(Container relativeAncestorContainer,
        ...
        SingleDockableContainer dockableContainer = null;
        if (relativeAncestorContainer.getComponentCount() == 0) { // empty docking panel
            return new DockingActionAddDockableEvent(desktop, dockable, initialState, nextState,
            ...
            if (split != null) {
                ...
                Rectangle splitRect = SwingUtilities.convertRectangle(split,
                        new Rectangle(0, 0, split.getWidth(), split.getHeight()), relativeAncestorContainer);
                ...
                if (split.getOrientation() == JSplitPane.VERTICAL_SPLIT) {
                    ...
                    if (anchoredTop) {
                        if (anchoredLeft) {
                            if (anchoredBottom) {
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, split, DockingConstants.SPLIT_LEFT, proportionW);
                            } else {
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, top, DockingConstants.SPLIT_LEFT, proportionW);
                            }
                        } else if (anchoredRight) {
                            if (anchoredBottom) {
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, split, DockingConstants.SPLIT_RIGHT, 1 - proportionW);
                            } else {
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, top, DockingConstants.SPLIT_RIGHT, 1 - proportionW);
                            }
                        } else {
                            // just anchored top
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, top, DockingConstants.SPLIT_TOP, proportionTopH);
                        }
                    } else if (anchoredBottom) { // but not anchoredTop
                        if (anchoredLeft) {
                            if (anchoredRight) {
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, bottom, DockingConstants.SPLIT_BOTTOM,
                                        1 - proportionBottomH);
                            } else {
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, bottom, DockingConstants.SPLIT_LEFT, proportionW);
                            }
                        } else if (anchoredRight) {
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, bottom, DockingConstants.SPLIT_RIGHT, 1 - proportionW);
                        } else {
                            // just anchored bottom
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, bottom, DockingConstants.SPLIT_BOTTOM, 1 - proportionBottomH);
                        }
                    } else if (anchoredLeft) { // not anchored on top / bottom
                        if (anchoredRight) { // 2005/10/10
                            // left + right on a vertical split, means we have to insert a new component
                            ...
                            if (bestComp == top) {
                                ...
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, bestComp, DockingConstants.SPLIT_BOTTOM, hChild, hParent);
                            } else {
                                float hChild = h / (float) splitBottom;
                                float hParent = yTop / (float) split.getHeight();
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, bestComp, DockingConstants.SPLIT_TOP, hChild, hParent);
                            }
                        } else { // only left
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, bestComp, DockingConstants.SPLIT_LEFT, proportionW);
                        }
                    } else if (anchoredRight) {
                        return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                nextState, bestComp, DockingConstants.SPLIT_RIGHT, 1 - proportionW);
                    } else { // not anchored at all, split verticaly and add
                        ...
                        if (bestCenter.y < centerY) {
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, bestComp, DockingConstants.SPLIT_BOTTOM,
                                    1 - proportionBottomH);
                        } else {
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, bestComp, DockingConstants.SPLIT_TOP, proportionTopH);
                        }
                    }
                } else {
                    ...
                    if (anchoredTop) {
                        if (anchoredLeft) {
                            if (anchoredBottom) {
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, left, DockingConstants.SPLIT_LEFT, proportionLeftW);
                            } else if (anchoredRight) {
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, split, DockingConstants.SPLIT_TOP, proportionH);
                            } else { // just anchored top/left
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, left, DockingConstants.SPLIT_TOP, proportionH);
                            }
                        } else if (anchoredRight) {
                            if (anchoredBottom) { // top + right + bottom
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, right, DockingConstants.SPLIT_RIGHT, 1 - proportionRightW);
                            } else { // top + right
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, right, DockingConstants.SPLIT_TOP, proportionH);
                            }
                        } else if (anchoredBottom) { // 2005/10/10
                            // top + bottom : create a horizontal split
                            // as we are inserting a vertical element into a horizontal split
                            ...
                            if (bestComp == left) {
                                ...
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, bestComp, DockingConstants.SPLIT_RIGHT, childW, parentW);
                            } else {
                                float parentW = xLeft / (float) split.getWidth();
                                float childW = w / (float) splitRight;
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, bestComp, DockingConstants.SPLIT_LEFT, childW, parentW);
                            }
                        } else {
                            // just anchored top
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, bestComp, DockingConstants.SPLIT_TOP, proportionH);
                        }
                    } else if (anchoredBottom) { // but not anchoredTop
                        if (anchoredLeft) {
                            if (anchoredRight) {
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, split, DockingConstants.SPLIT_BOTTOM, 1 - proportionH);

                            } else { // bottom + left
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, left, DockingConstants.SPLIT_BOTTOM, 1 - proportionH);
                            }
                        } else if (anchoredRight) { // bottom + right
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, right, DockingConstants.SPLIT_BOTTOM, 1 - proportionH);
                        } else {
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, bestComp, DockingConstants.SPLIT_BOTTOM, 1 - proportionH);
                        }
                    } else if (anchoredLeft) { // not anchored on top / bottom
                        return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                nextState, left, DockingConstants.SPLIT_LEFT, proportionLeftW);
                ...
                if (anchoredTop) {
                    if (anchoredLeft) {
                        if (anchoredBottom) {
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, singleComp, DockingConstants.SPLIT_LEFT, proportionW);
                        } else if (anchoredRight) { // top + left + right == TOP
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, singleComp, DockingConstants.SPLIT_TOP, proportionH);
                        } else { // top + left... what should we do ?
                            if (w > h) { // vsplit
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, singleComp, DockingConstants.SPLIT_TOP, proportionH);
                        ...
                        if (anchoredBottom) { // top + right + bottom == on the right
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, singleComp, DockingConstants.SPLIT_RIGHT, 1 - proportionW);
                        } else { // top + right
                            if (w > h) { // on top
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, singleComp, DockingConstants.SPLIT_TOP, proportionH);
                    ...
                    if (anchoredBottom) { // left + bot
                        if (anchoredRight) { // == bottom
                            return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                    nextState, singleComp, DockingConstants.SPLIT_BOTTOM, 1 - proportionH);
                        } else { // left + bottom
                            if (w > h) { // bottom
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, singleComp, DockingConstants.SPLIT_BOTTOM,
                                        1 - proportionH);
                            } else { // on the left
                                return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                        nextState, singleComp, DockingConstants.SPLIT_LEFT, proportionW);
                    ...
                    return new DockingActionSplitComponentEvent(desktop, dockable, initialState, nextState,
                            singleComp, DockingConstants.SPLIT_RIGHT, 1 - proportionW);
                } else { // no anchors
                    if (centerY < relativeAncestorContainer.getHeight() / 2) { // center is upper part
                        return new DockingActionSplitComponentEvent(desktop, dockable, initialState,
                                nextState, singleComp, DockingConstants.SPLIT_TOP, proportionH);
