 ...
 * @update 2005/12/08 Lilian Chamontin : fixed a bug related to multiple desktop usage (when moving a window after the
 *         desktop has been removed from hierarchy, the listeners were still invoked and caused a NPE in
 *         moveFloatingWindows).
 * 
...
public class DockingDesktop extends JLayeredPane {
    ...
    /**
     * a flag set when adding a maximized component : true is this one is heavyweight Only used with heavyweight support
                        ...
                        if (DockingPreferences.isLightWeightUsageEnabled()) {
                            ...
                            if (DockingPreferences.isSingleHeavyWeightComponent()) {
                                if (DockingUtilities.isHeavyWeightComponent(maximizedComponent)) {
                                    maximizedComponent.setBounds(i.left, i.top, w - i.left - i.right, h - i.top
                                        - i.bottom);
                                }
                                else {
     ...
     * 
     * Usage of this method should be limited to VLDocking extensions (simple users shouldn't rely on the underlying
     * DockingPanel existence)
     */
     ...
     * @param dockable
     *            a dockable to add at the same position than <code>base</code>. if base is not already child of a
     ...
     * 
     * @param base
     *            an existing dockable, either displayed in a DockableContainer or in a TabbedDockableContainer.
     *            <P>
    ...
    private void createTab(Dockable base, Dockable dockable, int order, boolean select, boolean triggerEvents) {
        ...
        if (baseTab != null) {
            ...
            DockingUtilities.replaceChild(((Component) baseOldContainer).getParent(), (Component) baseOldContainer,
                (Component) baseTab);
     ...
     * @param proportion
     *            proportion of the initial dockable space taken by the new dockable a negative proportion, like -1,
    ...
    public void split(Dockable base, Dockable newDockable, DockingConstants.Split position, double proportion) {
        ...
        DockableContainer dockableContainer =
            DockableContainerFactory.getFactory().createDockableContainer(newDockable,
        ...
        if (DockingPreferences.isLightWeightUsageEnabled()) {
            ...
            if (DockingPreferences.isSingleHeavyWeightComponent()) {
                ...
                if (DockingUtilities.isHeavyWeightComponent(comp)) {
                    ...
                    maximizedComponent.setBounds(i.left, i.top, getWidth() - i.left - i.right, getHeight() - i.top
                        - i.bottom);
                    add(maximizedComponent, JLayeredPane.PALETTE_LAYER);
        ...
        if (floating) {
            ...
            if (currentState != null && currentState.isFloating()) {
                if (previousState.isDocked()) {
                    ...
                    dae =
                        RelativeDockingUtilities.getInsertionDockingAction(relativeAncestor, dockable, currentState,
                            newState);
                }
    ...
    private void fireStateChanged(
        TabbedDockableContainer tdc, DockableState.Location futureLocation, RelativeDockablePosition position) {
        for (int i = 0; i < tdc.getTabCount(); i++) {
            Dockable d = tdc.getDockableAt(i);
            DockableState currentState = getDockableState(d);
            DockableState.Location currentLocation = getLocation(currentState);
            ...
            try {
                if (parent instanceof SplitContainer) {
                    ...
                    if (tparent.getTabCount() == 1) { // no more use for tabs
                        ...
                        if (floating) {
                            lastContainer =
                                DockableContainerFactory.getFactory().createDockableContainer(last,
    ...
    public void close(Dockable dockable) {
        ...
        if (dockingPanel.isAncestorOf(dockable.getComponent())) {
            ...
            if (isDockingActionAccepted(dae, dswe)) {
                ...
                context.setDockableState(dockable, new DockableState(this, dockable, DockableState.Location.CLOSED,
    ...
    void writeDesktopNode(PrintWriter out) throws IOException {
        ...
        out.println("<DockingPanel x=\"" + (int) windowLocation.getX() + "\" y=\"" + (int) windowLocation.getY()
            + "\" width=\"" + desktopWindow.getWidth() + "\" height=\"" + desktopWindow.getHeight() + "\"" + ">");
        if (dockingPanel.getComponentCount() > 0) {
    ...
    private void xmlBuildFloatingNode(NodeList dockables, Rectangle bounds) throws SAXException {
        ...
        for (int i = 0; i < dockables.getLength(); i++) {
            ...
            context.setDockableState(dockable, new DockableState(this, dockable, DockableState.Location.FLOATING,
                position));
            dockable.getDockKey().setLocation(DockableState.Location.FLOATING);
    ...
    private Component xmlCreateComponent(Element elt, DockableState.Location dockableLocation) throws SAXException {
        if (elt.getNodeName().equals("Split")) {
            ...
            SingleDockableContainer sdc =
                DockableContainerFactory.getFactory().createDockableContainer(d,
                    DockableContainerFactory.ParentType.PARENT_SPLIT_CONTAINER);
    ...
    private TabbedDockableContainer xmlBuildTabbedDockableContainer(Element elt, DockableState.Location dockableLocation)
        throws SAXException {
    ...
    public void replace(Dockable base, Dockable replacer) {
        ...
        if (tdcBase != null) {
            ...
            if (sdcBaseParent instanceof SplitContainer) {
                ...
                SingleDockableContainer sdc =
                    DockableContainerFactory.getFactory().createDockableContainer(replacer,
                        DockableContainerFactory.ParentType.PARENT_SPLIT_CONTAINER);
                sdc.installDocking(this);
                if (isLeft) {
