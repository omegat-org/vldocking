 ...
 * <p>
 * A "Drag Source" is a component, linked to a <code>Dockable<code>, responsible for drag gesture recognition.
 * There can be more than one DockableDragSource per Dockable.
 * <p>
 * For example, the title bar of a docked component is usually the drag source of this component (meaning :
 * when a user drags the title bar, he expects the dockable to be dragged around).
 * <p>
 * These draggable components must be able to give informations about their target (the component known to the
 * user, which is actually dragged).
 *
...
public interface DockableDragSource {
     ...
     * <P>
     * The source may reject the drag according to internal conditions (in that case this method shall return
     * <code>false</code>) or to wrong mouse position .
     *
